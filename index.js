...
console.log(fenomenler(fenomenler[5],profile));

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log(fenomenler(fenomenler[2],followers));
...
function indekseGoreFenomen(fenomenler,index) {
  var fenomen = fenomenler;
  var finx= fenomenler[index].profile;
  return `${index}. indekste bulunan fenomen: ${finx}`;
}
...
function profilListesi(fenomenler) {
 ...
 for (let i=0 ; i<fenomen.length ; i++){
    profilo.push(fenomen[i].profile);
   }
   return profilo;
 }
...
function fenomenSil(fenomenler,indeks) {
  var fenos = fenomenler;
  fenos.splice (index,0);
  return fenos;
